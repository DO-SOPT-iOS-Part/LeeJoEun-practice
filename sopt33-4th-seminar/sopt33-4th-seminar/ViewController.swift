...
class ViewController: UIViewController {
    ...
    @objc private func usernameCheck() {
        Task {
            print("==vc name: \(nickName)")
            if let result = try await GetUsernameCheckService.shared.PostRegisterData(username: self.nickName) {
                let isExit: Bool = result.isExist
                print("==isExit: \(result)")

                if (isExit) { //true 이름 겹침
                    let alert = UIAlertController(title: "사용자 이름 중복", message: "회원가입을 다시 해주세요.", preferredStyle: UIAlertController.Style.alert)
